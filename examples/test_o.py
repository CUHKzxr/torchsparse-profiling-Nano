...
def statis(list, model_type):
    ...
    return sum_backend[0].item() * raito, sum_backend[1].item() * raito, \
                sum_backend[2].item() * raito, sum_backend[3].item() * raito
...
def main():
    ...
    coords_df = pd.read_csv('/home/nano/torchsparse/data/'+sample+'/coords.csv',header=None)
    coords = torch.tensor(coords_df.values,dtype=torch.int).to(device)
    feats_df = pd.read_csv('/home/nano/torchsparse/data/'+sample+'/feats.csv', header=None)
    ...
    for backbone in [SparseResUNet42]:
        ...
        if(backbone is SparseResUNet42):
            read_weight_from_onnx(model, '/home/nano/torchsparse/data/'+sample+'/unet_v2.onnx', unet_dict(), device)
        else:
            read_weight_from_onnx(model, '/home/nano/torchsparse/data/'+sample+'/resnet_v2.onnx',resnet_dict_(), device)
        
        with open('./data/'+sample+f'/{backbone.__name__}.txt', 'a') as file:    
            file.write("=============================================================================================================" + '\n')
            file.write("start\n")
            ...
            print(np.mean(results,axis=0))
            print('average:', np.mean(results,axis=0), file=file)
            print(statis([], backbone))
            file.write("(gather, matmal, scatter, conv_backend total)\n")
            print(statis([], backbone), file=file)
            
            print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=100))
            print(prof.key_averages().table(sort_by="cpu_time_total", row_limit=100), file=file)
            prof.export_chrome_trace('./data/'+sample+f'/{backbone.__name__}_trace.json')
